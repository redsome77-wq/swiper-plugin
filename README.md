...
* 플러그인을 위해 쉽게 만드는 슬라이드 자바스크립트 동작
## Swiper 작성 준비
0. `head`안 `title` 아래 Swiper 사용을 위한 CND 연결
...
1. `html` 파일 내에서 슬라이드 삽입 위치에 다음과 같이 태그작성
    *  `div.swiper>div.swiper-wrapper>div.swiper-slide`
        ...
        * `div.swiper-slide` : 개별슬라이드들 개수만큼 `*숫자` 붙여서 생성하기
2. 1에서 생성한 태그에 대상구분용의 별도 클래스 따로 생성
    * `div.swiper.개별클래스명`
    * `<div class="swiper 개별클래스명">`
    * `div.swiper-slide.slide번호`
    * `<div class="swiper-slide slide1">`
3. 프로젝트 디자인에 따라 필요한 추가 태그들을 작성
    * `div.swiper-slide` 안에 `a, h, p, table, from, div 등...`
4. CSS 선택자 준비 (개별, 공통용 모두)
    **주의사항**`
    * `swiper-wrapper, swiper-slide`
        * **`display, width, hright, position` 적용하지 않기**
        * `swiper` 제일 바깥쪽 swiper시작태그한테만 크기 주기
...
* `loop` : 슬라이드 끝 -> 시작 반복 (true / false)
* `direction` : 수평(기본값)/수직 가능 ('horizontal', 'vertical')
    * 미설정시 이동방향 없이 제자리에서 전환!
* `effect` : 슬라이드 전환 애니메이션 ('fade', 'cube', 'flip')
* `mousewheel` : 마우스휠로 다음 슬라이드 전환. (true / false)
* `autoplay : {}` : 슬라이드 자동전환
    * `delay : 숫자` : 1000 = 1초
...
* 기본 순서 : `html -> js swiper options 세팅 -> (선택) css`
...
* `swiper-wrapper`, `swiper-slide`는 개별크기 입력하지 않기
...
### 교차 여부에 따라 위치를 다르게 해야 하는 이유
* `div.swiper`에 기본 css로 `overflow:hidden`가 있으므로 그 값을 `visible`로 바꾸면 선택요소 뿐 아닌 `swiper-slide`들도 모두 보이기 때문에 원하는 레이아웃을 만들 수 없음.
## Swiper 안에 추가 Swiper 생성시 안쪽 Swiper 마우스드래그 안될때 해결방법
...
1. `renderFraction:function(){}`
    ...
    * `const 변수명 = new Swiper(적용대상, {`
        ...
        `el: 적용대상,`
        `type: 'fraction'`
        `rederfraction: function(current, total{`
            `let 변수 = <태그 class=${매개변수}></태그>`
            `return 변수;`
        `}`
    `})`
## 슬라이드 변경 시 실행되는 함수 `on:{}`
* `new Swiper(대상, {옵션})` 옵션 위치에 바로 작성되는 함수
1. `init:function(){}`
